...
TFT_eSprite scrn = TFT_eSprite(&tft); //setup a sprite
...
void loop() {
  ...
  scrn.drawCentreString(String(sdTrigTime - timeNow) + " ms", 120, 20, 4);
  scrn.setTextColor(TFT_CYAN);
  scrn.drawString("# Writes", 0, 80, 2);
  scrn.setTextColor(TFT_SILVER);
  scrn.drawCentreString(String(sdWrite), 120, 80, 4);
  scrn.setTextColor(TFT_CYAN);
  scrn.drawString("Millis:", 0, 120, 2);
  scrn.setTextColor(TFT_SILVER);
  scrn.drawCentreString(String(timeNow), 120, 120, 4);
  scrn.pushSprite(0, 0);
  delay(1);
}
